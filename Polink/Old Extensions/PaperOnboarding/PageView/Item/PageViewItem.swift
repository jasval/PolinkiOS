...
extension PageViewItem {
    ...
    func animationSelected(_ selected: Bool, duration: Double, fillColor: Bool) {
		...
		if selected {
			scaleAnimation = circleScaleAnimation(0, duration: duration)
		} else {
			scaleAnimation = circleScaleAnimation(currentRadius - lineWidth / 2.0, duration: duration)
		}
...
extension PageViewItem {
    ...
    fileprivate func createBorderView() -> UIView {
		...
		var circleLayer : CAShapeLayer?
		if select == true {
			circleLayer = createCircleLayer(currentRadius, lineWidth: 0)
		} else {
			circleLayer = createCircleLayer(currentRadius, lineWidth: lineWidth)
		}
