...
class TextfieldView: UIView {
	...
	@objc func userDidTapButton() {
		if isTextInViewLongEnough() {
			textView.layer.borderColor = #colorLiteral(red: 0.1960784346, green: 0.3411764801, blue: 0.1019607857, alpha: 1)
			// Add animation
			delegate?.passTextfieldAnswerToViewController(textAnswer: textView.text, feedbackType: type)
			textView.endEditing(true)
		} else {
			// Add shaking animation
			textView.layer.borderColor = #colorLiteral(red: 0.720115006, green: 0, blue: 0.1111380085, alpha: 1)
			// Add Popup alert
		}
	}
